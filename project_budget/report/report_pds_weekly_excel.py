...
class report_pds_weekly_excel(models.AbstractModel):
    ...
    def get_etalon_project_first(self,spec):
        ...
        datesearch = date(YEARint, 1, 1)  # будем искать первый утвержденный в году
    ...
    def get_etalon_project(self, spec, quarter):
        ...
        datesearch = date(YEARint, 1, 1)
        if 'Q1' in quarter:
            datesearch = date(YEARint, 1, 1)  # будем искать первый утвержденный в году
        if 'Q2' in quarter:
            datesearch = date(YEARint, 4, 1)  # будем искать первый утвержденный после марта
        if 'Q3' in quarter:
            datesearch = date(YEARint, 7, 1)  # будем искать первый утвержденный после июня
        if 'Q4' in quarter:
            datesearch = date(YEARint, 10, 1)  # будем искать первый утвержденный после сентября
    ...
    def get_sum_fact_pds_project_step_week(self, project, step, week_number):
        global strYEAR
        global YEARint
    ...
    def get_sum_plan_pds_project_step_week(self, project, step, week_number):
        global strYEAR
        global YEARint
    ...
    def print_week_pds_project(self, sheet, row, column, week_number, project, step, row_format_number, row_format_number_color_fact):
        global strYEAR
        global YEARint

        sum75tmpetalon = sum50tmpetalon = sum100tmp = sum75tmp = sum50tmp = 0

        sum100tmp = self.get_sum_fact_pds_project_step_week(project, step, week_number)
        if sum100tmp:
    ...
    def print_row_Values(self, workbook, sheet, row, column,  YEAR, project, step):
        ...
        for week_number in range(1, date(YEARint, 12, 28).isocalendar()[1] + 1):
            ...
            # sheet.write_string(row, column + 0, "", row_format_number)
            # sheet.write_string(row, column + 1, "", row_format_number)
            # sheet.write_string(row, column, "", row_format_number_color_fact)
            # sheet.write_string(row, column + 1, "", row_format_number)
            ...
            # formula = '=sum({1}{0},{2}{0},{3}{0})'.format(row + 1, xl_col_to_name(column - 14),xl_col_to_name(column - 9), xl_col_to_name(column - 4))
            # sheet.write_formula(row, column, formula, row_format_number_color_fact)
            # formula = '=sum({1}{0},{2}{0},{3}{0})'.format(row + 1, xl_col_to_name(column - 13),xl_col_to_name(column - 8), xl_col_to_name(column - 3))
            # sheet.write_formula(row, column + 1, formula, row_format_number)
            # formula = '=sum({1}{0},{2}{0},{3}{0})'.format(row + 1, xl_col_to_name(column - 12),xl_col_to_name(column - 7), xl_col_to_name(column - 2))
